...
- **Solution**: Ensure your configuration includes the header `"Origin": "https://semobeta.vercel.app"`.

### 401 Unauthorized
This indicates an issue with your API Key.
- **Solution**: Verify that you are using a valid API Key from your [Dashboard](https://semobeta.vercel.app/api-keys) and that it correctly replaces `YOUR_API_KEY` in the configuration.

### Connection Failures
The AI client cannot establish a connection to the MCP server.
- **Solution**: 
  - Ensure the URL is exactly `https://semobeta.vercel.app/api/mcp`.
